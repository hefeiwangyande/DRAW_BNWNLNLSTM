...
import testutils
from testutils import  BasicLSTMCell,LSTMWNCell,LSTMBNCell,LSTMLNCell,LSTMCell
L1_REG_KEY = "L1_REG"
os.environ["CUDA_VISIBLE_DEVICES"] = "5"
tf.flags.DEFINE_string("data_dir", "data", "")
tf.flags.DEFINE_string("log_dir","LN_logs/", "")
tf.flags.DEFINE_boolean("read_attn", True, "enable attention for reader")
tf.flags.DEFINE_boolean("write_attn",True, "enable attention for writer")
FLAGS = tf.flags.FLAGS
if not os.path.exists(FLAGS.log_dir):
    os.mkdir(FLAGS.log_dir)
f=open(FLAGS.log_dir+'log.txt','w+')
log=Logger(FLAGS.log_dir+'log.txt')
import input_data
mnist = input_data.read_data_sets("/home/wangyan/mnist-master/Mnist_data/", one_hot=True)
...
# lstm_enc = BasicLSTMCell(enc_size, forget_bias=0.0, ln=True,state_is_tuple=True)
# lstm_dec = BasicLSTMCell(enc_size, forget_bias=0.0, ln=True,state_is_tuple=True)


lstm_enc = LSTMLNCell(enc_size, forget_bias=0.0, state_is_tuple=True,l1_reg=1e-2)
lstm_dec = LSTMLNCell(enc_size, forget_bias=0.0, state_is_tuple=True,l1_reg=1e-2)
...
def encode(state,input):
    ...
    with tf.variable_scope("encoder"):
        return lstm_enc(input,state,reuse=DO_SHARE,reuse_bn=None)
...
def decode(state,input):
    with tf.variable_scope("decoder"):
        return lstm_dec(input, state,reuse=DO_SHARE,reuse_bn=None)
...
if len(regularizer) > 0:
    ...
    cost = tf.add_n(regularizer) + _cost
else:
    cost = _cost
...
for i in range(train_iters):
	...
	if i%100==0:
         log.info("iter=%d : Lx: %f Lz: %f" % (i,Lxs[i],Lzs[i]))
...
out_file=os.path.join(FLAGS.log_dir,"draw_data.npy")
np.save(out_file,[canvases,Lxs,Lzs])
log.info("Outputs saved in file: %s" % out_file)

ckpt_file=os.path.join(FLAGS.log_dir,"drawmodel.ckpt")
log.info("Model saved in file: %s" % saver.save(sess,ckpt_file))
